  ...
  template: `
    @if (rows().length) {
      <div class="w-full">
        ...
        <div class="grid gap-1 w-full"
          ...
          @for (row of rows(); track row.day) {
            ...
            @for (cell of row.cells; track cell.hour) {
              <button type="button"
                      class="aspect-square w-full rounded-[3px] transition-transform
        ...
        <div class="flex items-center gap-2 mt-3 text-[11px] text-[color:var(--text-muted)]">
          <span>Less</span>
