 ...
 *
 * AppUserServiceImpl.scopedFind lets a tenant administrator reach the tenant users in its workspace and
 ...
 *
 * updateUser adds one rule that binds every actor including a platform administrator: on your own row,
...
describe('Users row actions', () => {
  it('lets a tenant administrator act on a tenant user in its workspace', () => {
    expect(usersFor('TENANT_ADMIN').canManage(rowFor(88, 'TENANT_USER'))).toBe(true);
  });

  it('offers a tenant administrator nothing on a peer administrator, which the server refuses', () => {
    expect(usersFor('TENANT_ADMIN').canManage(rowFor(77, 'TENANT_ADMIN'))).toBe(false);
  });

  it('keeps a tenant administrator able to edit its own row, admin role and all', () => {
    expect(usersFor('TENANT_ADMIN').canManage(rowFor(ACTING_ID, 'TENANT_ADMIN'))).toBe(true);
  });

  it('leaves every row open to a platform administrator, peers included', () => {
...
describe('UserDialog role picker', () => {
  it('offers a tenant administrator only the role it can grant', () => {
  ...
  it('offers a tenant administrator only its own role on its own row', () => {
  ...
  // platform administrator lets you post a different role onto your own row.
  it('offers a platform administrator only its own role on its own row', () => {
    expect(grantable(dialogFor('PLATFORM_ADMIN', { appUserId: ACTING_ID, userRole: 'PLATFORM_ADMIN' })))
      .toEqual(['PLATFORM_ADMIN']);
  });

  it('offers a platform administrator all three', () => {
    expect(grantable(dialogFor('PLATFORM_ADMIN')))
      .toEqual(['TENANT_USER', 'TENANT_ADMIN', 'PLATFORM_ADMIN']);
  });

  it('offers a platform administrator all three on somebody else, whatever they are now', () => {
...
describe('UserDialog access profile picker', () => {
  ...
  it('fetches the picked workspace\'s profiles for a platform administrator', () => {
